...
`Details') and the line characteristics \code{lend}, \code{ljoin} and
\code{lmitre}.
\end{ldescription}
\end{Arguments}
%
...
drawn.
\item[\code{col, lty, lwd}] graphical parameters, possible vectors.
\code{NA} values in \code{col} cause the arrow to be omitted.
\item[\code{...}] graphical parameters such as \code{xpd} and the line
...
\code{\LinkA{assoc}{assoc}} in the contributed package \pkg{vcd} (Meyer,
...
appropriate \code{axis} method (see \code{\LinkA{axis.POSIXct}{axis.POSIXct}}).
\end{Details}
%
\begin{Value}
...
\item[\code{...}] other graphical parameters may also be passed as
...
\aliasA{plot.Date}{axis.POSIXct}{plot.Date}
\aliasA{plot.POSIXct}{axis.POSIXct}{plot.POSIXct}
\aliasA{plot.POSIXlt}{axis.POSIXct}{plot.POSIXlt}
...
\item[\code{y}] numeric values to be plotted against \code{x}.
\item[\code{xlab}] a character string giving the label for the x axis.
...
\code{\LinkA{plot.default}{plot.default}}.  For the \code{plot} methods, also
\code{format}.
\end{ldescription}
\end{Arguments}
%
...
As from \R{} 2.9.0 the date-times for a \code{"POSIXct"} input are
interpreted in the timezone give by the \code{"tzone"} attribute it
...
\item[\code{usr}] numeric vector of length four, defaulting to
\code{\LinkA{par}{par}("usr")} giving horizontal (`x') and vertical
...
is \code{TRUE}, see the documentation on \code{\LinkA{par}{par}(xaxp=.)}.
...
\code{\LinkA{axis}{axis}(side)} would use or has used.
\end{Value}
%
\begin{SeeAlso}\relax
...
parameters (see \code{\LinkA{par}{par}}) which are passed to
...
\item[\code{...}] further graphical parameters, such as \code{bty},
...
passed to \code{\LinkA{bxp}{bxp}} in addition to the ones
...
%
\begin{Author}\relax
Martin Maechler, 1995, for S+, then \R{} package \pkg{sfsmisc}.
\end{Author}
%
\begin{SeeAlso}\relax
...
\item[\code{pars,...}] graphical parameters (etc) can be passed as
...
\begin{Arguments}
\begin{ldescription}
\item[\code{x}] an object, the default method expects either a single numerical variable.
...
typically graphical parameters such as \code{cex.axis}.
\end{ldescription}
\end{Arguments}
%
\begin{Details}\relax
...
\pkg{lattice} package where the \code{\LinkA{formula}{formula}} notation allows
...
\code{\LinkA{levelplot}{levelplot}}) from package \pkg{lattice}.
...
\begin{Arguments}
\begin{ldescription}
\item[\code{expr}] an expression written as a function of \code{x}, or
...
\item[\code{xlim}] numeric of length 2; if specified, it serves as default
for \code{c(from, to)}.
...
\item[\code{ylab, log, ...}] labels and graphical parameters can also be
...
\end{Examples}
\inputencoding{latin1}
\HeaderA{dotchart}{Cleveland Dot Plots}{dotchart}
...
\item[\code{...}] graphical parameters can also be specified as arguments.
\end{ldescription}
\end{Arguments}
%
\begin{Value}
...
\item[\code{...}] additional graphical parameters, currently only passed to
...
\code{\LinkA{levelplot}{levelplot}} and
\code{\LinkA{contourplot}{contourplot}} functions from the
\pkg{lattice} package which work in multipanel displays.
...
There is a hook called \code{"plot.new"} (see \code{\LinkA{setHook}{setHook}})
...
\item[\code{right}] logical; if \code{TRUE}, the histograms cells are
...
character argument.
\item[\code{...}] further arguments and graphical parameters passed to
...
\code{\LinkA{density}{density}},  \code{\LinkA{truehist}{truehist}} in package \pkg{MASS}.
...
\item[\code{...}] graphical parameters, or arguments to
...
\item[\code{...}] graphical parameters for \code{\LinkA{plot}{plot}} may also be
...
\item[\code{xpd}] if supplied, a value of the graphical parameter \code{xpd}
...
\code{\LinkA{mosaic}{mosaic}} in the contributed package \pkg{vcd} (Meyer,
Zeileis and Hornik, 2005).
\end{Details}
%
\begin{Author}\relax
...
\item[\code{at}] give location in user coordinates.  If \code{length(at)==0}
...
names are currently taken from the unexported variable \code{.Pars}.
...
There are several parameters can only be set by a call to \code{par()}:
\begin{itemize}
...
\code{bg} and \code{cex}, which may be taken as arguments to certain
...
since the parameters were saved.  You will reset all of \code{mfrow},
\code{mfcol} and \code{mfg} and will find \code{mfrow} wins.
...
Note that the \code{adj} argument of \code{\LinkA{text}{text}} also
...
packages \pkg{grid} and \pkg{lattice}.  It can be set even on
...
functions such as \code{\LinkA{points}{points}} accept a vector of values
which are recycled.  Other uses will take just the first value if
a vector of length greater than one is supplied.

This starts as \code{1} when a device is opened, and is reset when
the layout is changed, e.g. by setting \code{mfrow}.
...
(Some functions such as \code{\LinkA{lines}{lines}} accept a vector of values
which are recycled.  Other uses will take just the first value if
a vector of length greater than one is supplied.)
...
Also supported by \code{\LinkA{mtext}{mtext}}.  Note that other
string/character rotation (via argument \code{srt} to \code{par})
...
querying cannot tell you the order the array will be filled.
...
drawing \emph{as if it was on a \bold{\emph{new}} device}.  It is
...
normalized device coordinates), i.e., as fraction (in \eqn{[0,1]}{})
...
(\code{tck = NA}) is to use \code{tcl = -0.5}.
...
\item[\code{...}] additional graphical parameters (see \code{\LinkA{par}{par}}).
\end{ldescription}
\end{Arguments}
%
\begin{Details}\relax
...
\code{"cex.axis"}/\code{"font.axis"} respectively.  (This changed in
\R{} 2.5.0.)
\end{Details}
%
\begin{Value}
\code{persp()} returns the \emph{viewing transformation matrix}, say
\code{VT}, a \eqn{4 \times 4}{} matrix suitable for projecting 3D coordinates
\eqn{(x,y,z)}{} into the 2D plane using homogeneous 4D coordinates
\eqn{(x,y,z,t)}{}.
It can be used to superimpose additional graphical elements on the 3D
plot, by \code{\LinkA{lines}{lines}()} or \code{\LinkA{points}{points}()},
using the simple function \code{\LinkA{trans3d}{trans3d}()}.
\end{Value}
%
\begin{References}\relax
Becker, R. A., Chambers, J. M. and Wilks, A. R. (1988)
...
\item[\code{...}] graphical parameters can be given as arguments to
...
\item[\code{...}] Arguments to be passed to methods, such as graphical
parameters (see \code{\LinkA{par}{par}}).
...
\item[\code{panel.first}] an expression to be evaluated after the plot axes
...
\item[\code{asp}] the \eqn{y/x}{} aspect ratio, see \code{\LinkA{plot.window}{plot.window}}.
\item[\code{...}] other graphical parameters (see \code{\LinkA{par}{par}} and
...
\item[\code{lty}] the line type, see \code{\LinkA{par}{par}}.
...
\item[\code{lwd}] the line width, see \code{\LinkA{par}{par}}.
...
%
\begin{Description}\relax
Plot univariate effects of one ore more \code{\LinkA{factor}{factor}}s,
...
\item[\code{...}] graphical arguments such as \code{col}, see \code{\LinkA{par}{par}}.
...
\code{\LinkA{plot}{plot}} as appropriate.  All of these accept graphical
parameters, see \code{\LinkA{par}{par}}, annotation arguments passed
...
\code{horizontal = TRUE} can be used (see \code{\LinkA{boxplot}{boxplot}}).
\end{Details}
%
\begin{Value}
...
\item[\code{...}] further graphical parameters to \code{title} and \code{axis}.
\end{ldescription}
\end{Arguments}
%
\begin{Details}\relax
...
\end{Examples}
\inputencoding{latin1}
\HeaderA{plot.table}{Plot Methods for `table' Objects}{plot.table}
...
\item[\code{asp}] numeric, giving the \bold{asp}ect ratio y/x, see below.
\item[\code{...}] further graphical parameters as in \code{\LinkA{par}{par}}.
...
\item[\code{...}] further graphical parameters such as \code{xpd},
...
\item[\code{type}] character indicating the type of plotting; actually any of
the \code{type}s as in \code{\LinkA{plot.default}{plot.default}}.
\item[\code{...}] Further graphical parameters may also be supplied as
...
latter uses a border and so is perceptibly larger when \code{lty} is
...
to that of \code{\LinkA{lines}{lines}}, except
...
\code{\LinkA{pdf}{pdf}} and \code{\LinkA{postscript}{postscript}} devices
...
Prior to \R{} 2.8.0 \code{rug} re-drew the axis like: it no longer does so.
\end{Details}
%
\begin{References}\relax
...
\item[\code{col, lty, lwd}] graphical parameters as in
\code{\LinkA{par}{par}}, possibly vectors.  \code{NA} values in \code{col}
cause the segment to be omitted.
\item[\code{...}] further graphical parameters (from \code{\LinkA{par}{par}}),
...
%
\begin{Details}\relax
Where an element of \code{s} is a multi-line string (that is, contains
newlines \samp{\bsl{}n}), the width and height are of an enclosing
rectangle of the string as plotted by \code{\LinkA{text}{text}}.  The
...
(`petals') such that overplotting is visualized instead of accidental and
invisible.
\end{Description}
%
\begin{Usage}
...
\code{number[i]} equi-angular `rays' emanate from it.
...
\code{\LinkA{jitter}{jitter}} the orientations of the sunflowers in order to prevent
...
\end{Examples}
\inputencoding{latin1}
\HeaderA{symbols}{Draw Symbols (Circles, Squares, Stars, Thermometers, Boxplots) on a Plot}{symbols}
...
graphical parameters in \code{\LinkA{par}{par}()}.
\item[\code{...}] further graphical parameters (from \code{\LinkA{par}{par}}),
...
\code{\LinkA{par}{par}}); this rotates about the centre set by \code{adj}.
...
\item[\code{...}] further graphical parameters from \code{\LinkA{par}{par}}.  Use
...
\item[\code{...}] graphical parameters such as \code{lty}, \code{xpd},
